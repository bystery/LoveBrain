...
> ⚠ **下一个窗口（第 12 个）请先读 `LoveBrain_Handover_Window12_StartHere_2026-09-26.md`** ——
> 那份是自包含的开工包（指导书行号地图 + 剩余工作四组清单 + 起手命令 + 硬约束），不需要先读完本文件。
> 本文件继续作为过程档案：格子增量、被证伪的旧话、坑表都在这儿。
...
## 0.40 §6.1 :490 的第二把尺收进仓库，口径变严：23 处 / 10 文件（`1f980d4`）

- 旧实现是仓库外的 `_temp/scan_primary_buttons.py`：不进 CI、没证人、没正向对照，而且只认
  "`clickable` 自己那条链上有品牌底色" ⇒ `d0b6358` 把面板那两颗改成热区/视觉分两层之后它从 17 掉到 15，
  **一处债没还、数字自己降**。现在尺在 `core/testing/SourceScan.kt`、判定在
  `UiLayerDependencyContractTest`，**逐文件等号证人**（只核总数会被"一家还、一家长"糊过去），
  尺自己另有 `SourceScanTest` 十格对照；python 那把退役（不删）。
- 新尺多认两种形状：热区分层（外层可点、里层涂底）、以及 **clickable 在外层品牌盒的尾随 lambda 里**
  —— 后者第一发也漏过：`Box( … ) { … }` 的内容在配对右括号**之外**，得把花括号块算进子树（`callEnd`）。
- ⚠ **口径变了，判决范围就得跟着重算**：23 处 / 10 文件里有 8 处是旧尺看不见的 ⇒
  本文件那句":490 已逐处判完"**只对旧 15 处成立**。本格没把"判完"扩到新清单，
  下一格按行号表逐处判语义（唯一主动作 / chip / 次级 / 选中态底色），别按数量收口。
- ⚠ 引信一条（今天没咬到）：`codeOf()` 删注释**不区分字符串**，`ProviderSection:390` 那句
  `placeholder = "https://api.example.com"` 里的 `//` 会吃掉该行后半段；本机扫过 `ui/` 只这一条、
  且该行无别的判据形状 ⇒ 现无伤，**统一那两把老尺时要顺手换掉**（新尺的 `maskComments` 已经对了）。
- 实测：见 §1 基线行；尺与闸全绿，一次性诊断与退役脚本都在 `_temp/gates107/`。

## 0.41 :490 新增可见的 8 处判完：0 处该搬，但照出 5 条 :531/:532 缺口（`1f980d4`）

- 逐处判决与理由在**账本 §56 那张表**。一句话：这 8 处没有一处是"那一页的唯一主动作"
  （浮球 / 整卡折叠 / 整卡激活 / 导出 / 追问 / 两颗已修的 chip 与 ➕ / 一颗本来就合规的过滤页签）
  ⇒ **:490 的"该不该搬"对 23 处这份清单判完了**，本格**没改生产**。
- 判的过程照出 5 条**别的性质**的缺口：`FeedbackCasesScreen:201`、`ProviderSection:142`、
  `KnowledgeBaseActivity:412`、`CounselingPanel:477` 四处 `clickable` 没声明 `role`（:532），
  其中 `CounselingPanel:477` 还**完全没有热区下限**（只有 `clip` + `background`）；
  以及 `FloatingBubble:258` 那颗 `clickable(onClick = { })` 是**空 lambda**——
  树里"能点却什么都不做"，`performClick` 静默无效、读屏念得出按钮却点不出东西（坑表 113）。
- ⚠ 这些**不当成本格的修法**：先挂进语义树量一遍再改（三处所在屏已有 JVM 夹具，
  追问那颗要先给面板整屏一个"有回答结果"的档位）。浮球那颗要不要撤 clickable 是**交互归属判断，没自签**。
- ⚠ 本文件之前那句":490 已逐处判完"（⑫/⑬ 那两处）现在**已按 23 处的口径重述**，
  旧口径下那句只对 15 处成立（坑表 111）。

## 0.42 量完之后：§56 那张表窄了——反馈案例页 9 颗可点**全部 `role=无`、6 颗低于 48**

- §56 的判决表是**读源码**写的（"这处没 role、那处没热区"）。这一格把三屏挂进语义树量了一遍，
  结论是"推断方向对、范围小一大截"，明细与判读在**账本 §57**。三句话：
  ①反馈案例页 9 颗可点**全部 `role=无`**、其中 6 颗热区只有 15–19dp 高；
  ②那一页的页头是 `LbTopBar` 归一（`d6c546a`）**留下的第五式**：`「←」48x48dp role=无`，
    名字就是箭头字形本身 ⇒ 英文环境也念不出"返回"；
  ③知识库整卡（激活）`312x136dp role=无`、供应商整卡（折叠）`312x53dp role=无`、
    行内「编辑」/「导出」`50x48dp role=无`（尺寸达标、缺角色）。
- ⚠ **整卡在树里的名字不是卡片标题**（知识库整卡第一个文本是「阶段： ｜ 已对话 0 轮」）
  ⇒ 守卫认"整卡那颗"要按**最宽的可点节点**认，按 label 认会认错人（坑表 114）。
- 排定下一格（先量后改、都不自签）：**把反馈案例页接进 `LbScreenScaffold` + `LbTopBar` 并补齐
  那 6 颗热区**——按记忆里那句"搬那一页要一次销两笔"（表面色表一行 + 页头一处），别只销一半；
  第二格再判三处整卡与两颗行内按钮该报 `Button` 还是 `Tab`（看是否互斥可重复，不照抄）。
- 本格**不改生产**（诊断格），一次性诊断收档 `_temp/gates107/`。

## 0.43 两格收口：出口判据三发修到 verify 全绿；反馈案例页一次销两笔（`04259ef`/`6f0bc6e`/`8456199`）

- **出口那一格（§2 的 verify 通道）修了三发才绿**，每一发都是上一发修好才看得见的：
  ① fixture 的 pcap magic 写成带内校验和变体，tshark 拒收，而生成器自带的解析器
  **比的是同一个错常数**（本机永远绿）；② 判据脚本自己在 `set -e` + `pipefail` 下
  **静默中止**（`grep -v` 空选、`getent` 查不到主机），退出码像判决、报告一个字没写，
  其中两格**恰好**拿到期望的 exit 1 = 假绿长在期望 FAIL 的格子上；
  ③ 白名单判据写成 `[ "$name" = "$h" ] || case …ok=1…`，**相等时短路右边不执行** ⇒
  完全匹配的主机反被判成表外（SNI/DNS 各抄一遍，一次点亮两条）。
  ⇒ CI run 36208159365：`verify` **33 步全跑、0 skipped、success**（历史上第一次；
  R8 release、APK metadata、SBOM、取消审计、工单号、费用基线、三个 Upload 全是第一次真跑到）。
  明细与本机证据在**账本 §58.1–58.2**；坑表 116–118。
- **反馈案例页搬完，两笔豁免同时清空**：外框 `LbScreenScaffold`（水平边距量到 12dp → 24dp）、
  页头 `LbTopBar`（返回那颗现在念 `R.string.common_back`，不再念「←」），
  六颗芯片改成两层（外盒 48×48 + `Role.Tab` + `selected`，视觉那颗胶囊一个字没改），
  整卡与页头那颗导出补 `Role.Button`。改前改后同一台仪器对照表在**账本 §58.3**。
- ⚠ **一条被证伪的旧理由**：页头那一格原来写着"这一页要 `rememberLauncherForActivityResult`，
  JVM 挂不起来"——**挂着量到了**，挂不起来的是触发那一步。坑表 120。
- 两台仪器的锚点被这一页打掉重画：`PageHeaderConsistencyTest.headerControl`
  （现在"先最靠上、并列才比左"，两版错法都留了读数）；
  `SemanticsProbe` 新增 `laid/unlaid`（横排滚出视口那颗在树里是 `0x0 @(0,0)`，
  会把"最左"顶成 0dp）。**判尺寸要么给够宽度、要么横扫取最大面积，两档都要有格子。**
- P5 那一发第一读 **NO-TEETH**：整屏底色那把尺只认 `background(SurfaceBase`，
  具名实参与全限定两种写法都看不见 ⇒ "刚删掉豁免的闸正好是瞎的"。尺已改口径 + 加三档正向对照（`8456199`），
  复跑 BIT。坑表 119：**销账之后必须把债重新借回去试一次。**
- 下一格（新窗口开工就接着做，账本 §58.9 有清单）：
  ①反馈案例页的 **Loading / Error / Empty 三档还没量**（§6.3 那四态这一页只齐了 Content）；
  ②§57.3 排定但故意分开的那格——三处整卡（知识库激活 / 供应商折叠 / 案例展开已顺手做完）
  与知识库行内「编辑」「导出」两颗的 role，判 `Button` 还是 `Tab` 要先看互斥性，**量完再定**；
  ③出口那一格留的尾巴：`--capture-on-device` 那半截从没在任何机器上跑到过
  （第 108 行那个 adb tcpdump 的 EXIT trap 会被后面的 `trap … EXIT` 顶掉，本机无设备核不了，
  账本 §58.9 记了一行）。

## 0.44 一把新尺：P0-03 前半句第一次被数出来——写链 4 / 裸写 30（`aa29950`）

- **指导书 217 行那半句「所有 mutation 只能从 `KnowledgeTx` 取得安全路径与原子写能力」，
  此前没有任何一格在数。** 上一轮以为"读写边界已清零"依据的是另外两把尺
  （`rawAtomicWriteText` 只有一条链、`File(File(knowledgeRoot,` owners 已空集），
  两句都不管"谁绕过 `KnowledgeTx` 直接调 `atomicWriteText`"。⇒ 本格只造尺、不改生产码。
- 实扫登记（`KnowledgeTxMutationEntryTest`，8 格）：**总数 34 = 唯一写链 4 + 裸写 30**，
  裸写落在 `create` 13 / `ensureInitialKnowledgeBase` 13 / `ensureKbFilesCompleteUnlocked` 2 /
  `RepoStorage.atomicWrite`+`guardedWrite` 各 1。总数、两堆分类、**逐作用域明细表**三个判据一律 `==`。
  另钉三条：定义必须留 `private`、不许出现 `::atomicWriteText` 引用、`data/` 别家不许调用。
- **八发变异反证全符合预期**（`_temp/a1_mutate/readings.txt`，每发还原后 sha256 逐字节核对）：
  最值钱的是 M1——把一处裸写挪进写链，**总数一点不动**，只有分类与明细表红；
  两发正向对照（注释里写满调用形状、多调一次 `rawAtomicWriteText(`）必须全绿才算数。
  明细与三处仪器形状坑在**账本 §59**。坑表 121–123。
- ⚠ 这把尺钉的是**结构债不是漏洞**：30 处仍过 `atomicWriteText` 里那道按 `kbOwning` 的只读判定，
  `create` 也自己洗库名（`/`、`\`、`.` 全剥），别把它读成"30 个可绕过的洞"。
- 本机收口：12 步门禁全 rc=0，**1417 单测 / 190 套件 / 0 失败 0 错误 0 跳过**
  （基线 1409/189 涨的 8 格+1 套件全来自新尺），lint 读数与预算登记逐字相同（新格一条 lint 没添）。
- **下一格候选（按"能不能马上动"排）**：
  ①还债第一刀——从 `ensureKbFilesCompleteUnlocked` 那 2 处开手（最小的一族），改完回来把三个数改小；
  ②开工单 3.A2 极端值矩阵（¥9999.999 / 100000 次，全仓现在一格都没有）；
  ③账本 §59.6 新登记的那条行为空白：只读未来库走"补缺文件"那条路径时的目录树比对**没量过**。

## 0.45 销两笔"注释比实现新"的假账，顺手挖出两笔只登记没自裁的（`d6dc1a1`）

- **①跨层存量的注释落后了两次还债**：`PackageDependencyTest` 的 KDoc 写"15 条 / 11 个文件"，
  实测两把尺都是 **6**（`package_deps_report.sh --count` = 6、它自己那份 baseline 六个键，
  `assertEquals(6, …)` 每轮在核）。还债发生过两次且提交信息里都写了数
  （`2dd94c0` 15→10、`ab7457a` 10→6），**只有注释没跟着动**；
  同段另一处"把 domain 的 6 处 `data.*` 换掉"也已经换完了（domain 现在只剩一处 `android.content.Context`）。
- **②验收包 :76"仍用冻结 prompt"当场改口**（指导书 §3.3 点名的两句之一）。
  改之前对着实现核过：`GenerationEngine.kt:256-266` 的注释自己就写着"那是没有实现的说法"，
  hash 漂移只 `L.w`，随后送出的就是**现读资产拼的文本**。
  ⚠ 同一句要求的 **:81"所有写路径统一拒"故意没动**（开工包明写别先改字，而 A1 那 30 处裸写一处没还）。
- **挖出来两笔，只登记不自裁**（账本 §60.3）：
  ①`…Execution_Log…:49` 写"'仍用冻结 prompt' 已改口"——被改口的是**引擎注释**，
  那份验收包文件 `git log` 显示上次动它停在 `c0ff041`（**注释改口 ≠ 文档改口**）；
  ②同一行又说 :81 那句"现在变成真的了"，与开工包"A1 修完才成立"**互相矛盾**。
  摊开口径：库内落盘全部过 `atomicWriteText` 那道 `kbOwning` 判定 = 真；
  "mutation 只能从 `KnowledgeTx` 取能力" = 还差 30 处。**两个都对，不许我择一自签**。
- 顺带又被死导入那把尺抓到一条（`Assert.fail`，**本格改之前就已死**）；lint 至今抓不到这类。
- 本机读数：`PackageDependencyTest` 六格改注释前后各 `--rerun` 一次全绿；
  全套 12 步门禁 `_temp/run_gates110.sh` **全 rc=0**：**190 套件 / 1417 单测 / 0 失败 0 错误 0 跳过**
  （与上一格 `aa29950` 逐字相同——本格只动注释与一条死导入，格子数本来就不该动）；
  lint `67/15`、进预算 `66/14`、advisory 1（与开工单 §6 逐字相同）；跨层 `--count` = 6；
  预算自测 27 格、工单号、prompt 零 diff、资产锁 `6dcde732…`、取消审计 165 站、`assembleAndroidTest` 全过；
  死导入两个文件各 `4 explicit imports, 0 unused`。账本 **§60**。坑表 124–125。
...
最近一轮实测基线（到 `d6dc1a1`，与上一笔 `aa29950` **逐字同数**——`d6dc1a1` 只动注释与一条死导入）：**1417 单测 / 190 套件 / 0 失败 / 0 错误 / 0 跳过**
（+8 格 +1 套件 = 新尺 `KnowledgeTxMutationEntryTest` 那八格，账本 §59）；
再往上是 `8456199` **1409 / 189**、`1f980d4` 1399/188、`d0b6358` 1388/187、`16e4bd6` 1384/186。
（`8456199` 那一步的 +10 = `FeedbackCasesSemanticsTest` 新套件八格 + `PageHeaderConsistencyTest` 第五页 +
`ScreenScaffoldFrameTest` 第四家，账本 §58。）
⚠ 数单测一律用 `--rerun` + 看全部 XML 是不是同一时间戳（本次 189 份同批、跨度 0.04s）——
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **还剩的账不是"没判"，是"判了、该不该搬"**：这些自造按钮现在热区与角色都达标了，
     ...
     能按下去的自造按钮 **23 处 / 10 文件**（逐处判完见 §0.41 与账本 §56；新尺在仓库里：`core/testing/SourceScan.kt` + 闸 `hand-drawn brand-toned actionable widgets do not grow`，**逐文件**等号证人；`_temp/scan_primary_buttons.py` 已退役（没删）；
     ⚠ **17→15 一处债都没还**：少的两条正是 `ReplyInput:123`/`:219`，`clickable` 被我挪到外层盒子之后
     这把尺的"同一条链上既有 clickable 又有品牌底"就看不见它们 ⇒ **这是尺瞎了，不是进展**；
     统一三把尺那一格要把"热区与视觉分两层"这种形状认下来——**这件事做了**（`1f980d4`，见 §0.40）：新尺同时认"同链 / 两层 / 尾随 lambda / 条件涂色"四种形状，⚠ **换口径之后 23 > 15 是尺变严，不是债涨了**，那句"已逐处判完"只对旧 15 处成立）⚠ 上一格登记的 18 是 `856d485` **之前**的读数、更早的 19 是 `bba6159` **之前**的读数：那颗「下一步」归进 `LbPrimaryButton` 之后就不在这把尺里了，**代码还了债、表没跟着改**（坑表 71 那一族的反向复发）。
     ⚠ **而"这把尺没剥注释、读的是裸原文"那句是错的，已更正**：旧 python 脚本第一步就调 `mask_comments`（它还做了"偏移不变所以行号可信"）。这一档真正的瞎点是**形状**（同链 / 两层 / 尾随 lambda），不是注释——**口径写错会把下一格引到错的方向上修**（坑表 108 的又一发）。新尺的掩码按字符换空格、长度不变，而且**分字符串**：`codeOf()` 那份不区分字符串，`"https://…"` 里的 `//` 会吃掉该行后半段（`ProviderSection:390`，今天没咬到判据，见 §0.40 的引信条）。
     **换一扇门涂色的 `containerColor = <品牌色>` 2 处 / 2 文件**（`_temp/scan_container_color2.py`，剥注释 + 括号配对口径；见下面 ⑬）。
     ...
     ⇒ ~~**:490 那张清单到此全部判完**~~ —— 这句**当时只对旧口径的清单成立**；
     新尺（`1f980d4`）之后清单变成 23 处，新增可见的 8 处已在 `1f980d4` 逐处判完（账本 §56，
     结论：0 处该搬），现在这句才是对**当前清单**的判完（5 处 Material 换门 → 2 处，剩下两处都不是主动作：
...
## 6. 坑表（编号连续：1–15 上一份，16–25 CI 首跑，26–31 画像格，32–35 回滚与只读，36–44 归档/状态统一/无障碍，45–52 四态与输入框，53–54 搬家与两把尺，55–57 状态表与异步收尾，58 引用了≠用上了，59–60 语义树锚点与变异归因，61–63 搬家照出的三把瞎尺，64–65 变异工具自己的两个坑，66 文案藏在默认实参里，67 恢复要点名、同一目录可能有第二个写者，68 「取第一个非空」的判据会被别的来源蹭过去，69 界面构造不出的状态要对着持有者测，
    70–89 尺自己瞎了的第二茬，90–107 滚动/组件内边距/交付物里的假事实/门禁自己空跑，124–125 假账销账与口径冲突不许择一自裁，126–128 合并树里的锚点/贴着 teardown 的假相关/只数成功的尺，129–131 恒假的协议退出判据/不等就读的异步计数断言/退出码 0 + 文件非空仍什么都不断、132 坏实现对照的基线写成活引用、133 默认模式只重录不比对
    ⚠ 正文按**加入顺序**排，不严格递增（102 后面接着 95、90 那批是补记的）——
    要按号找条目就搜 `^\d+\. `，别假设它是升序的。**编号到 133**
...
111. **尺变严之后，之前那句"逐处判完"就只对旧清单成立了**（`1f980d4`）：
     同一件事（:490 "自画的品牌底可点控件"）三档口径给出三个数：链上同色 17 → 分两层后旧尺 15 →
     新尺 **23**。前一个是"代码还了债、表没跟着改"，第二个更阴：**代码没动，数字自己降**——
     因为形状变了（热区与视觉分两层），旧尺的认法看不见。
     ⇒ 换尺/改形状时要当场问：**这个数是尺变的还是代码变的？** 办法只有一个——
     把两把尺的差集打出来逐条看（本格打了行号表）。
     并且"判完/清完"这类结论要**随口径重新声明**：新尺多认 8 处，那句"判完"就只对旧 15 处成立，
     不重判就不许把 23 念成"又还了 8 处"。

112. **按形状认的尺，"范围"要在两处一起对：注释要掩对、边界要算进尾随 lambda**（`1f980d4`）：
     ①`codeOf()` 那类"删注释"实现**不区分字符串**，`"https://…"` 里的 `//` 会把行后半段吃掉
     （今天只在 `ProviderSection:390` 擦肩而过，没咬到判据）；掩码要**按字符换成空格**、
     长度不变，行号才可信（坑表 94 那一族）。
     ②`Box( … ) { … }` 的内容在配对右括号**之外**——只按右括号取"子树"，
     "外层可点 + 里层涂底"与"外层涂底 + 里层可点"两种形状都会漏。
     ⇒ 这类尺的正向对照必须**每种形状各一条**（本仓库现在有：同链 / 两层 / 尾随 lambda / 条件涂色），
     少一种就等于那一族代码没有闸。

113. **`clickable(onClick = { })` 空 lambda 是一颗"看得见点不着"的节点**（`1f980d4` 判浮球那一处时照出来）：
     `FloatingBubble` 的球自己挂了 clickable，只为了收集按压态，真实点击由父级的拖拽判定——
     语义树里它却是一颗**有点击动作、什么都不做**的节点：`performClick` 静默无效（坑表 76 那一族：
     点了不等于点中），读屏念得出"按钮"却什么也不发生。
     ⇒ 收集按压态不必借道 `clickable`（`Modifier.pointerInput` 或父级回调都行）；
     要留着就得让它**真的做事**或至少不冒充动作。判这类节点用两条一起看：
     `hasClickAction()` 有没有、点了之后语义/回调有没有变——只看第一条会把"假按钮"读成"能按"。
     撤掉它还是改归属是**交互判断，没自签**，本格只登记。

114. **整卡可点那颗在语义树里的名字不是卡片标题**（`role_gap_probe` 三屏实量）：
     知识库那张整卡 merge 之后第一个文本是「阶段： ｜ 已对话 0 轮」，供应商整卡是「未配置供应商」，
     案例整卡甚至只剩「【】」（卡片里那行空标题）。
     ⇒ 守卫要认"整卡那颗"，判据应是**这一屏最宽的可点节点**（`ProviderSectionSemanticsTest.clickCard()`
     早就这么写，理由是同一个）；按 label 字符串认会认错人——
     认错之后那条断言照样能绿（它量的是别的节点），这比红更难发现。

115. **读源码得出的缺陷表，范围一定偏窄**（`§56 → §57` 这一对）：
     §56 逐处读代码写下"这处没 role、那处没热区"，量完之后同一页变成
     **9 颗全 `role=无` + 6 颗热区 15–19dp**，还多出一处"页头第五式"（`「←」`当名字）。
     ⇒ 缺陷表要**由语义树生成**，源码只用来定位怎么改；
     已经按源码判过的事，挂上仪器之后要**重跑同一批站点**（本仓库第三次撞到这条：
     `91babe7` 接 ResultArea、`16e4bd6` 接表单/编辑屏、这次接三屏，每次读数都比表宽）。

116. **写方与读方不许共用同一个常数**（出口 fixture 的 pcap magic，`207ebd1` 之前）：
     生成器把全局头写成 `0xA1B2C213`（带内校验和的 pcap 变体），而它自带的 `inspect()`
     比的**正是同一个错常数** ⇒ 本机复述内容全对、CI 上 tshark 一句"不是我能懂的抓包文件"全废。
     ⇒ 派生物（fixture、快照、生成的报告）必须有一个**不是自己写的读者**：
     本机这次用的是 `file`（libmagic）——修前 5 个全报 `data`，修后 5 个报
     `pcap capture file, microsecond ts (little-endian)`，一句话就把自证拆了。
     同一族还有一支：**差分桩不许假装会 dissect**，它只需要照真工具的"行形状"吐数据，
     作用是复现控制流；真解析的判决仍只能等 CI（这一格就是这么分两发修的）。

117. **`set -euo pipefail` 会把"探测命令的正常非 0"变成静默中止**（出口判据，`04259ef` 之后那发）：
     `grep -v` 一行都没选中返回 1、`getent hosts <查不到的主机>` 返回 2，两处都没有兜底 ⇒
     脚本当场死，**退出码长得像判决、报告一个字没写**。最阴的读数是其中两格
     恰好拿到了期望的 exit 1 —— **假绿长在"期望 FAIL"的格子上**，
     只核对退出码的自测试完全看不出来（要配"报告里那一行的条数"这类第二证人）。
     ⇒ 探测类命令的"没结果"要用**永不失败的工具**表达（`sed /d` 而不是 `grep -v`），
     空不空交给后面的 `[ -s ]` 判；再加一条 `set -E` + ERR 兜底：
     脚本没打印任何判决就死掉时，一律改写 CANNOT-VERIFY 并把退出码钉在 2。

118. **`[ "$x" = "$y" ] || case …置位… ;; esac` 这种写法在相等时不置位**（出口白名单判据）：
     `||` 短路，右边那句根本不执行 ⇒ **完全匹配的那颗主机反被判成表外**，只有子域放过。
     同一句在 SNI 与 DNS 两派各抄一遍，于是一个 bug 点亮两条视角。
     ⇒ 判"命中"要写成显式的 `if 相等 then 返回命中`，别让"置位"只挂在短路右边；
     两条视角共用一条判据时就合成一颗函数（本仓库第 N 次撞到"同一句话抄两遍"）。

119. **从表里删掉一条豁免之后，必须当场注入一次"新违规"确认闸还看得见**（P5 第一读 NO-TEETH）：
     页面外框那把尺写的是 `code.contains("background(SurfaceBase")`，
     于是 `background(color = SurfaceBase)` 与全限定写法**都看不见**——
     我刚把那一页的豁免删掉，同一发的检查就证明新违规照样放行。
     **一条刚被"销账"的闸如果是瞎的，比留着那条豁免更危险**（下一个人会以为管住了）。
     ⇒ 每次销账都要配一发"把债重新借回去"的变异；按形状认的尺还要写**正向对照**
     （几种写法都认得 + 一个不该认的必须不认）。

120. **写在豁免注释里的"做不了"，越具体越容易骗过自己**：页头那一格的注释点名说
     "这一页要 `rememberLauncherForActivityResult`，**JVM 上挂不起来**"——挂不起来的是
     **触发**那一步（要起真 Activity 选择器），注册与组合都没问题，§57 的探针就是这么量到节点的。
     一条没验过的具体理由给一笔欠账续了三个窗期的命。
     ⇒ 凡是把"没做"归因于"做不了"，落笔时同时写下**我是怎么知道的**；
     答不出来就先试十分钟，试完把读数写进注释（与坑表 87、115 同一族）。

121. **会自己数"作用域"的尺，要先清同级兄弟——体无花括号的声明永远等不到属于自己的那个 `}`**（`aa29950`）：
     第一版只在 `}` 时出栈，于是 `private fun isWritable(k): Boolean = …` 赖在栈上，
     把 `writeFileUnlocked` 报成 `isWritable.writeFileUnlocked`、把 `RepoStorage.atomicWrite`
     报成三个函数名叠在一起。**总数 34 对、分类 4/30 对、两格全绿，只有明细表是错的**——
     一张登记错家的表会让人带着"这 30 处在这几个地方"去还债，找错门还算得通。
     ⇒ 压栈前按"声明深度 ≥ 当前深度"清一遍（同级互清）；并且**明细表本身要钉成断言**，
     只钉两个总数拦不住"位置错了但数没变"这一整类。

122. **"另写一把尺交叉核对"这事本身要先体检：分步正则剥注释/字符串会整片吞代码。** 同一格：
     python 复算尺按"剥块注释 → 剥行注释 → 剥字符串"三步走，报 **13 处**、行号还整体错一行——
     因为剥完行注释后某行剩下奇数个引号，字符串正则就从那个引号一路配对吞掉后半个区间的代码。
     如果当时信了"两把尺不一致＝主尺虚高"，就会去把 34 改小，那是**反向的假绿**。
     ⇒ 复算必须与主尺同结构（单遍状态机、按字符 1:1 抹平、行号不动）；
     这轮真正用的交叉核对是最笨那条：`grep -n` 直接列出的 34 个行号与尺报的行号**逐个对上**。
     同一族的第三条形状：`rawAtomicWriteText(` 把 `atomicWriteText(` 整个包住，
     数调用点要查"前一个字符不是标识符字符"，定义行要按 `fun <名字` 捕获组下标剥掉，
     两道**各配一发正向对照**（多调一次 raw、注释里写满调用形状，两发都必须全绿）。

123. **一句要求里的两半句要两把尺；"某条已清零"必须回去对着原文逐字找尺，不能对着工具名。** 第 N 次：
     P0-03 的修复方式一句里含两事——"字节只从一条链落盘"（`rawAtomicWriteText` 那把尺管着，已达标）
     与"mutation 只能从 `KnowledgeTx` 取得能力"（**一把尺都没有**，实扫 30 处裸写）。
     上一轮拿前一把尺的绿灯回答了后一把尺的问题。
     ⇒ 收口时把要求原文拆开逐句对上"哪一格在数它"，说不出格名就是没覆盖；
     还债类计数还要能看见**搬家**：M1 那发证明"从裸写堆挪进写链"总数一点不动，
     只钉总数的棘轮对这类债完全是瞎的。


124. **"已改口 / 已修正"这类话必须点名到哪一份文件，否则它会把"注释改了"说成"文档改了"**（`d6dc1a1`）：
     执行记录里有一行写"'仍用冻结 prompt' 已改口"，读起来像那份验收包已经修好；
     实际被改口的是**引擎源码的注释**，`git log --oneline -- LoveBrain_Rework_Acceptance_2026-09-24.md`
     显示那份文件上一次被改动停在 `c0ff041`，**比那行日志还早** ⇒ 那句假话在文档里又活了两天。
     同一格里另一笔：KDoc 写"数字来自实扫，不是凭印象写的"，而实扫数早就从 15 变到 6（两次还债，
     提交信息都写了数），**只有注释没跟着动**——"这句自证现在两边都不成立"。
     ⇒ ①凡"某处已改口/已清零"，落笔时同时写**哪一份文件、哪一次命令输出**；
     ②还债之后回扫的顺序是：代码 → 测试基线 → **注释/KDoc/文档里的存量叙述** → 日志措辞；
     ③证伪成本极低：`git log -- <那个文件>` 一行。

125. **两份文档对同一句话判决相反时，摊开口径、不许择一自裁**（`d6dc1a1`）：
     关于 `:81`"所有写路径统一拒"，开工单说"A1 修完之后才成立，别先改字"，
     执行记录说"现在**变成真的**了"。两边都对——只是量的不是同一层：
     库内落盘 34 处全在 `atomicWriteText` 那道按 `kbOwning` 的判定下游（**出口口径＝真**），
     而"所有 mutation 只能从 `KnowledgeTx` 取得安全路径与原子写能力"还差 30 处裸写（**入口口径＝不成立**）。
...
133. **roborazzi 默认模式"只重录不比对"——把单测跑绿当成看过视觉回归，等于没装这道门**（`0bc1597`）：
     注入一次真实的视觉回归（主按钮 `heightIn(min=)` 48dp → 64dp）之后，
     `:app:testDebugUnitTest` **rc=0 全绿**，而 `:app:verifyRoborazziDebug` **三格全红**。
     默认模式下 roborazzi 只把"实到图"写进 build/，不跟基线比；⚠ 但它也不覆盖
     `app/src/test/roborazzi/` 里的基线（跑完单测三个哈希未变）——所以危险不是"自动改基线"，
     而是"CI 里有一步绿着，人就以为这件事被看着了"。
     ⇒ ①比对必须**独立成一步**（CI 里只跑 verify，永不跑 record）；
     ②新增工具型门禁时，先量一次"**它不出力时，同一份坏实现会不会绿**"，
     答案是"会"就得把那条独立步骤写成硬性（并在这一步的产物里留下比对报告）。
     同轮两条形状：compose API 的真实签名要先 `javap` 看清（1.30 是 `captureRoboImage { content }`
     包住内容的 composable，不是 `SemanticsNodeInteraction` 扩展，凭习惯写就编译拒绝）；
     传相对文件名会把图落到**模块根目录**（`app/xxx.png`）而不是比对目录——用默认命名。
