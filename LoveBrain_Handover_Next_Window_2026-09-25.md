...
最近一轮实测基线（到 `5477762`，`--rerun` 强制跑完后按 mtime 核过 183 份 XML 同批）：
**1362 单测 / 183 套件 / 0 失败 / 0 错误 / 0 跳过**；上一格到 `c202da2` 是 1353/182（+1 套件、+9 格）。
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **还剩的账不是"没判"，是"判了、该不该搬"**：这些自造按钮现在热区与角色都达标了，
     ...
     能按下去的自造按钮 **18 处 / 8 文件**（`_temp/scan_primary_buttons.py`，本次重扫）。⚠ 登记的 19 是 `bba6159` **之前**的读数：那颗「下一步」归进 `LbPrimaryButton` 之后就不在这把尺里了，**代码还了债、表没跟着改**（坑表 71 那一族的反向复发）。
     ...
     **换一扇门涂色的 Material `Button(containerColor = Primary)` 4 处 / 4 文件**（`c202da2` 把 `KnowledgeBaseActivity:324` 
     ...
     ~~**剩 3 处仍未判**~~ —— 第 1 处**已经量到手**（`5477762`，账本 §46.2）：
     表单「保存」实量 `role=Button 160x48dp`、名字来自内联文案、**达标**，
     ⇒ 它这一屏确实是唯一主动作（同一 Row 里「取消」是次级），**该不该搬进 `LbPrimaryButton`**
     是下一格的归属判断（搬过去还能把现在手写的 `enabled` + `if (saving) spinner` 收成一颗旋钮）。
     **剩 2 处连挂都没挂起来**：`KbEditActivity:474`、`KnowledgeBaseActivity:780`——
     每颗要先判"是不是那一页的唯一主动作"，看着像不等于量过。
