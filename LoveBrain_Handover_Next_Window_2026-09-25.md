...
最近一轮实测基线（到 `07e1463`）：**1377 单测 / 185 套件 / 0 失败 / 0 错误 / 0 跳过**；
往上依次是 `f5d199d` 1370/184、`5477762` 1362/183、`c202da2` 1353/182。
...
**TEXT 182 / DESC 11 / STATE 0 / COMPONENT 77**，合计 270；
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **换一扇门涂色的 `containerColor = <品牌色>` 2 处 / 2 文件**（`_temp/scan_container_color2.py`，剥注释 + 括号配对口径；见下面 ⑬）。
     ...
     ⑯ **`LbPrimaryButton` 只保证一条边**（`07e1463`，账本 §50.3）：组件原来写
     `.height(48)`，「保存」搬进去之后短标签量出 **33x48dp** ⇒ 改成 `heightIn(min) + widthIn(min)`
     写回组件本身，另加一格四态都跑的见方下限断言。这条与 `LbTextAction` 注释里那句
     「只垫高度不够，短标签会量出 40x48dp」是同一课——**同一课在两个组件上各缺了一次**，
     说明这类「下限」要写在组件里、并由语义树判，不能靠调用方记得给宽度。
