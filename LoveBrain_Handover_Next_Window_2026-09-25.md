...
最近一轮实测基线（到 `f5d199d`）：**1370 单测 / 184 套件 / 0 失败 / 0 错误 / 0 跳过**；
上一格到 `5477762` 是 1362/183，再上一格 `c202da2` 是 1353/182。
...
**TEXT 183 / DESC 11 / STATE 0 / COMPONENT 78**，合计 275；
（187→185 是表单「保存」两条中文进资源；185→183 是问卷那两颗主动作的标签进资源。
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **还剩的账不是"没判"，是"判了、该不该搬"**：这些自造按钮现在热区与角色都达标了，
     ...
     表面色 **45 处 / 17 文件**（旧口径 25 是下界，**别拿 45 与 25 比涨跌**）、
     能按下去的自造按钮 **18 处 / 8 文件**（`_temp/scan_primary_buttons.py`；**这把尺本次没跟着改口径**，见下一行的 ⚠）⚠ 登记的 19 是 `bba6159` **之前**的读数：那颗「下一步」归进 `LbPrimaryButton` 之后就不在这把尺里了，**代码还了债、表没跟着改**（坑表 71 那一族的反向复发）。
     ...
     **换一扇门涂色的 `containerColor = <品牌色>` 3 处 / 2 文件**（`_temp/scan_container_color2.py`，剥注释 + 括号配对口径；见下面 ⑬）。
     ...
     新闸 `brand tones painted through containerColor do not grow` 现在登记 **3 处 / 2 文件**；
     ...
     剩这一处要判：`KbEditActivity` 那颗保存。**门槛已经查明不是「要 VM」**：
     `KbEditScreen(files, lastFile, onLastFileChange, readFile, saveFile, onBack)`
     收的全是 lambda，没有 ViewModel ⇒ 挂进仪器只是把 `private` 改 `internal`（照 
     `ProviderFormBody` / `OnboardingScreen` 的旧路走）。同屏 4 颗 `TextButton` 一起量。
