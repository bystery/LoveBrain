...
> **本文件的读法**：§0.1–§0.30 是一格一段的增量（§0.30 最新，`3e1f074`：设计系统五颗手画组件全都缺 `role`，逐颗量完补上），§1 起手命令与现在值，
...
最近一轮实测基线（到 `3e1f074`）：**1346 单测 / 180 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1341 → 1346 = +5，179 → 180 套 = +1
（+5 = `DesignSystemRolesTest` 五格，一颗组件一格）。
