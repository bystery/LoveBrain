...
> **本文件的读法**：§0.1–§0.24 是一格一段的增量（§0.24 最新，`efefef4`），§1 起手命令与现在值，
> §2 被证伪的旧话（含"注释承诺了一道不存在的闸"那类），§4 下一格顺序（6 = §6.1 那张表走到 11/11 组件齐、但"顶部栏"那件还没归一，
> 6b = 便宜穿插格），§5 已做勿重复，§6 坑表（**76–77 最新**：量的判据要挑必然在树里且边界清晰的
> 节点（布局容器不在语义树里）、抄来的事实和量出来的在文档里长得一样；74–75 是"永远不可能咬的探针"与"慢一帧量到空树"）。
> 账本最近三节：§34 §6.4 第四刀、§35 §6.4 第五刀、§36 勘误 + §6.1 最后一行
> （含"两条把用户关在浮层里的死路"与"穷尽 when 编译器已守着、尺子真正防的是 else 吞态"那两笔）。
...
最近一轮实测基线（到 `a78d029` + `efefef4`）：**1320 单测 / 174 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1316 → 1320 = +4，173 → 174 套 = +1
（+4 = `ScreenScaffoldFrameTest` 3 格量边距 + `the page frame has exactly one owner` 1 格）。
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **顺带量到并修掉两条会把用户关在浮层里的死路**（见 §0.23——那两条比搬家本身重要）。
     ...
     ⑦ **§6.1 那一行还剩「顶部栏」没归一**（`efefef4` 只收了背景与水平边距两件）。
     今天实测**四式并存**：`LbTopBar`（只有首页）、`ScreenHeader`（`ScreenPage` 系列 +
     知识库编辑）、手写 `←`+标题的 `Row`（关于 / 使用概览 / 供应商）、
     `SurfaceCard` 底手写栏（反馈案例）。下一格若还在 §6.1 主线上，就做这一件——
     它同时能把 `ScreenHeader` 与 `LbTopBar` 两个名字收成一种。
     ⑧ `FeedbackCasesScreen` 那 1 处整屏底色是**闸里登记着的欠账**：
     它内部区块自己带 `Spacing.lg` 边距，直接套脚手架会"外面 24 里面又 12"叠两层，
     要连着内部边距一起改。**本格没量它的外框边距**（那页要
     `rememberLauncherForActivityResult`，JVM 挂载另说），所以不替它报数；
     **还掉之后必须把 `registeredDebt` 里那一行删掉**——判的是 `==`，不删会红。
     ⑨ 首次引导那颗主按钮实量 **328x34dp**，不到 §6.5 :531 的 48dp。量到了、没顺手改。
