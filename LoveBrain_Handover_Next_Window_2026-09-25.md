...
> **本文件的读法**：§0.1–§0.27 是一格一段的增量（§0.27 最新，`08b762d`），§1 起手命令与现在值，
> §2 被证伪的旧话（含"注释承诺了一道不存在的闸"那类），§4 下一格顺序（6 = §6.1 那张表：11 颗组件齐、:478 与 :479 都已归一、:490 开了两处，只剩 insets 待设备定，
> 6b = 便宜穿插格），§5 已做勿重复，§6 坑表（**83 最新**：两把尺正对着干——收数之后"要求每处自己抄 48"的静态尺集体 `was null`；
...
> 账本最近三节：§37 §6.1 页头归一、§38 §6.1 :490 逐处判读、§39 那颗 48 写了 17 遍 + 页级文字动作补上主人。
...
最近一轮实测基线（到 `08b762d`）：**1335 单测 / 177 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1330 → 1335 = +5，176 → 177 套 = +1
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **一处字面量 + 14 处引用**（`ProductionUiContractTest` 三条"要求每处自己抄 48"的尺
     ...
     ⑪b A 堆那 **7 处量不到所以一处没动**（`SuggestPanel:217/:261`、`CounselingPanel:205`、
     `ResultArea:340/:1230`、`PrimaryLight` 那三处小面块）：那些页要么要 VM、要么从没在 JVM 挂过。
     **先接页面再判这几颗**，别拿"看起来是主动作"当证据搬。
     ⇒ 这是 §6.1 主线上**下一格该做的事**：接 `ResultArea`/`SuggestPanel` 进 JVM 仪器。
     顺带解锁的不止这 7 处——§6.5 :531/:532 那些性质今天也只覆盖了挂得起来的那几页。
     ...
     **换一扇门涂色的 Material `Button(containerColor = Primary)` 5 处 / 4 文件**
     ...
     ⑬ **:490 的清单漏了一整族**（`08b762d` 顺手照出来，账本 §39.7）：那把尺锚在
     Modifier 链的 `.background(品牌色)` 上，而 Material `Button` 的底色走
     `ButtonDefaults.buttonColors(containerColor = Primary)`，**整族没进清单**。
     实扫 `Button(` 7 处、带品牌色 5 处，其中 `HomeComponents:227` 就是**首页那颗唯一主按钮**
     ——它现在不归 `LbPrimaryButton`，而 :479 那行要的正是"页面唯一主动作"。
     ⇒ 先判这 5 处各自该不该搬，再考虑给那把尺补第三个锚点（属于"还债后计数没动 = 尺在漏"那族）。
     ⑭ **`MiniSwitch` 的注释是假的**（`ProviderSection.kt`，同一格顺手照出来）：写着
     "扩大到 48×32，**满足** 48dp 无障碍下限"，而 :531 要 `bounds ≥48×48`，它是 `toggleable`、
     高度 32。本格只把假话改成实话、把那一处内联 `48.dp` 留在棘轮里**标成已知缺陷**——
     ⚠ **这条只读了声明，没在语义树量过**。供应商页在 §37 那格已挂进过仪器，
     下一格先量它，再决定是垫高度还是换 Material `Switch`。
...
83. **两把尺可以正对着干，收数那一格必须先回扫"要求抄数"的尺**（`08b762d`）：
    ...
    ⇒ 收"重复的数"这种全局动作，落刀前先扫一遍**谁在按字面量读它**（`--tests` 全量跑就是这张网）；
    改尺的时候不许把"读不出数"直接判过（本项目那条"体检取不到数要失败"仍然成立）。
    顺带一笔：这种时刻**极容易顺手把断言改软成"能读到就算过"**——这一格明确没做，
    改成"算得出 ≥48 才算过"，并新钉了 81 说的那个承重点。
