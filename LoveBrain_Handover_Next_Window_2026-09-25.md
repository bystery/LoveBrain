...
> **本文件的读法**：§0.1–§0.23 是一格一段的增量（§0.23 最新，`67c8dfa`），§1 起手命令与现在值，
> §2 被证伪的旧话（含"注释承诺了一道不存在的闸"那类），§4 下一格顺序（6 = §6.1 剩余 8 行，
> 6b = 便宜穿插格），§5 已做勿重复，§6 坑表（**74–75 最新**：永远不可能咬的"探针"不是反证、
> 先组合后改状态会慢一帧量到空树；72–73 是筛键与被测性质互相干扰那一族）。
> 账本最近三节：§33 §6.4 第三刀、§34 §6.4 第四刀、§35 §6.4 第五刀
...
最近一轮实测基线（到 `6d2d45c` + `67c8dfa`）：**1316 单测 / 173 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1307 → 1316 = +9，171 → 173 套 = +2
（`RecordSentFailurePathTest` 2 格 + `RecordSentFlowTest` 7 格）。两笔提交中途的读数是 1309，
差的 7 就是第二笔那七格——**逐笔要能对上，对不上就是有人加了套件却没加格**。
