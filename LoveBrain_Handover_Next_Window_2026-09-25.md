...
> **本文件的读法**：§0.1–§0.29 是一格一段的增量（§0.29 最新，`4ee1514`：首页主按钮归位 + 量出设计系统自己缺的角色），§1 起手命令与现在值，
...
> 6b = 便宜穿插格），§5 已做勿重复，§6 坑表（**87 最新**："改用统一组件"这一步自己会引入语义回归（Material 自带 role，手画的没声明），
...
最近一轮实测基线（到 `4ee1514`）：**1341 单测 / 179 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1338 → 1341 = +3，178 → 179 套 = +1
（+3 = `HomeHeroActionTest` 两格 + `brand tones painted through containerColor…` 一格）。
