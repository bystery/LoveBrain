...
> **本文件的读法**：§0.1–§0.25 是一格一段的增量（§0.25 最新，`d6c546a`），§1 起手命令与现在值，
> §2 被证伪的旧话（含"注释承诺了一道不存在的闸"那类），§4 下一格顺序（6 = §6.1 那张表：11 颗组件齐、:478 与 :479 都已归一，只剩 insets 待设备定，
> 6b = 便宜穿插格），§5 已做勿重复，§6 坑表（**78 最新**：清死导入的自查工具自己也会误判——
> 运算符导入按"名字没出现"判死是错的；76–77 是量的锚点与抄来的事实）。
> 账本最近三节：§35 §6.4 第五刀、§36 勘误 + §6.1 页面外框、§37 §6.1 页头归一
> （含"页头四式里两派说不出自己"与"有一条字面量归不出它原来在哪一栏，明写不编解释"那两笔）。
...
最近一轮实测基线（到 `d6c546a`）：**1327 单测 / 175 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1320 → 1327 = +7，174 → 175 套 = +1
（+7 = `PageHeaderConsistencyTest` 6 格页头 + `the page header has exactly one owner` 1 格）。
