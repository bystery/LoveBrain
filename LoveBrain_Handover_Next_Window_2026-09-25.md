...
最近一轮实测基线（到 `6910098`）：**1181 单测 / 147 套件 / 0 失败 / 0 错误 / 0 跳过**
（最旧 XML 05:36:20，本次跑起点 05:33:20 由 shell 记下——`BUILD SUCCESSFUL in 3m` 这种无秒数的写法
会让"从日志时长反推起点"的正则落空，两种都要能读）；
lint 报告**重新生成后**（05:39）70 条 / 15 规则，其中 **进预算 69 条 / 14 规则**、advisory 1 条；
...
`LoveBrainViewModel` 2746 → 2756 → 2732 → **2739** 行；
VM 里私有 `MutableStateFlow` 用同一把尺量：**39 → 31 → 30**（上一笔我写的"39→30"是正则漏匹配，见坑表 41）。
...
## 6. 坑表（编号接上一份的 1–15；16–25 CI 首跑后那批，26–31 画像格那批，32–35 回滚与只读那批，36–42 归档与状态统一那批）
