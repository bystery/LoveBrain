...
> **本文件的读法**：§0.1–§0.28 是一格一段的增量（§0.28 最新，`91babe7`：`ResultArea` 第一次挂进 JVM 仪器），§1 起手命令与现在值，
> §2 被证伪的旧话（含"注释承诺了一道不存在的闸"那类），§4 下一格顺序（6 = §6.1 那张表：**表里 11 行全有主**、
> 另补一颗 `LbTextAction` 承接表内"可选文字动作"那半句；:478/:479 已归一、:490 搬三处、那颗 48 收成一处，只剩 insets 待设备定，
> 6b = 便宜穿插格），§5 已做勿重复，§6 坑表（**86 最新**：钉数的老尺会把缺陷一起钉死，改成钉性质当场红；
...
> 账本最近三节：§38 §6.1 :490 逐处判读、§39 那颗 48 写了 17 遍 + 页级文字动作、§40 ResultArea 接进仪器一次现形 14 个。
...
最近一轮实测基线（到 `91babe7`）：**1338 单测 / 178 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1335 → 1338 = +3，177 → 178 套 = +1
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **一处字面量 + 14 处引用**（`ProductionUiContractTest` 三条"要求每处自己抄 48"的尺
     ...
     ⑪b A 堆那 **7 处**量不到所以一处没动 → **`91babe7` 销掉 2 处**（`ResultArea` 挂进仪器，
     一次量到 14 个不达标节点、全修：见 §0.28 与账本 §40）。**还剩 5 处**：
     `SuggestPanel:217/:261`、`CounselingPanel:205` 等——这两颗**是真的** `viewModel:
     LoveBrainViewModel` 入参（不像 `ResultArea` 那样是我误记的"要 VM"）。
     ⇒ 下一格要接的是"能给出一颗可构造的 VM 或把 Koin 图接进 JVM"这条路，
     而**不是**再猜一次"它挂不起来"（这个错在 §40.1 刚犯过一次并被拆掉）。
     顺带欠着：`ResultArea` 的 `providerReady = false` 那一档、以及本格三格只在 600dp 一档跑过
