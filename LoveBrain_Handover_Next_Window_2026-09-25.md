...
> **本文件的读法**：§0.1–§0.26 是一格一段的增量（§0.26 最新，`bba6159`），§1 起手命令与现在值，
> §2 被证伪的旧话（含"注释承诺了一道不存在的闸"那类），§4 下一格顺序（6 = §6.1 那张表：11 颗组件齐、:478 与 :479 都已归一，只剩 insets 待设备定，
> 6b = 便宜穿插格），§5 已做勿重复，§6 坑表（**79 最新**："往前看 N 个字符"不能当同一条链、
> 剥注释会毁掉行号偏移；78 是清死导入的自查工具自己也会误判——
> 运算符导入按"名字没出现"判死是错的；76–77 是量的锚点与抄来的事实）。
> 账本最近三节：§36 勘误 + §6.1 页面外框、§37 §6.1 页头归一、§38 §6.1 :490 逐处判读
> （含"两把尺扫出 25 与 19 两个数都对"、"第一版清单 26 处全是假的"那两笔）。
...
最近一轮实测基线（到 `bba6159`）：**1330 单测 / 176 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上一格对账：1327 → 1330 = +3，175 → 176 套 = +1
（+3 = `OnboardingPrimaryActionTest` 2 格 + `hand-drawn brand-toned surfaces do not grow` 1 格）。
   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **搬它那一页要一次把两笔都销掉，别销一半**；两把闸都判 `==`，
     ...
     ⑪ **§6.1 :490 才开了头**（`bba6159` 只搬掉一处，判读全表在账本 §38.2）。两条要继续的：
     ⑪a **页级弱化文字动作没有所有者**——首次引导那颗「跳过」现在是借
     `LB_SHEET_ACTION_MIN_DP` 垫到 48dp 的。48dp 这个下限目前在
     `LB_PRIMARY_MIN_HEIGHT_DP` 与 `LB_SHEET_ACTION_MIN_DP` **各有一份**，该收成一颗组件+一个常量。
     ...
     **先接页面再判这几颗**，别拿"看起来是主动作"当证据搬。
     ⑫ 两把尺各扫各的，别再合成一个数：表面色 **25 处 / 12 文件**（棘轮登记的就是这把），
