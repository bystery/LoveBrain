...
> **本文件的读法**：§0.1–§0.31 是一格一段的增量（§0.31 最新，`aff3edf`+`7b6ce9c`：那颗 48×32 的开关量到手 + 一把会把还债吃掉的尺），§1 起手命令与现在值，
...
最近一轮实测基线（到 `7b6ce9c`）：**1348 单测 / 180 套件 / 0 失败 / 0 错误 / 0 跳过**
（跑在变异全撤之后的树上）。与上两格对账：1341 → 1346（`DesignSystemRolesTest` 五格）
→ 1348（`MiniSwitchRow` 两格）。
⚠ 字面量预算这四个数**换口径重测过**（`7b6ce9c` 起先剥注释再数）：
**TEXT 188 / DESC 11 / STATE 0 / COMPONENT 78**，合计 277；
