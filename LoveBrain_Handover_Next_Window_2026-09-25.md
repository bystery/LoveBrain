   ...
   **但"有主人"不等于"那行做完了"**，两件要紧的还欠着：
   ...
   - **§6.4 剩下的真账**（`67c8dfa` 之后重扫过。① 已做；原先挂在末尾那句没写完的
     ...
     **还剩的账不是"没判"，是"判了、该不该搬"**：这些自造按钮现在热区与角色都达标了，
     ...
     能按下去的自造按钮 **23 处 / 10 文件**（新尺在仓库里：`core/testing/SourceScan.kt` + 闸 `hand-drawn brand-toned actionable widgets do not grow`，**逐文件**等号证人；`_temp/scan_primary_buttons.py` 已退役（没删）；
