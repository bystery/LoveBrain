 ...
 *
 * 审计修复：测试直接使用生产 ReplyPrimaryActions 组件（reply/ReplyPrimaryActions.kt），
 * 不再使用 ReplyPrimaryActionsTestable 复制品。消除"测试替身与生产逻辑出现文字差异"。
 *
...
class ReplyPrimaryActionsTest {
    ...
    @Test
    fun replyMode_noResult_zeroMessages_showsGenerateReplyDisabled() {
        composeRule.setContent {
            ReplyPrimaryActions(
                composerMode = ComposerMode.REPLY,
                isGenerating = false,
