...
class GenerationEngine(
    ...
    interface Callbacks {
        ...
        // ═══ F07: 换个思路·方向生成 ═══
        /** 方向生成开始 */
        fun onDirectionStart() {}
        /** 方向生成流式 schemes 回调 */
        fun onDirectionStreaming(schemes: List<Scheme>) {}
        /** 方向生成成功——更新 result 中的 directions */
        fun onDirectionResult(schemes: List<Scheme>) {}
        /** 方向生成失败 */
        fun onDirectionError(error: String) {}
        /** 方向生成结束 */
        fun onDirectionEnd() {}
        /** 是否正在生成方向 */
        fun isDirectionGenerating(): Boolean = false
     ...
     *
     * 复用本轮冻结上下文（同一 messages/userHint/kb/intent/corrections），
     * 仅更换 system prompt 为 direction.md，输出任务为生成四方向。
     * 不再在普通四风格请求中生成 directions。
     *
     * @param messages 冻结的本轮消息快照
     * @param userHint 冻结的 IDEA hint
     * @param knowledgeBase 冻结的 KB 快照
     * @param scope ViewModel scope
     * @param callbacks Engine 回调
     * @param intentConfig 冻结的持续意图
     * @param corrections 冻结的纠正记录
     * @param onlyThisRound 是否仅看本轮
     */
    fun generateDirection(
        ...
        if (messages.isEmpty() || callbacks.isDirectionGenerating()) return null

        callbacks.onDirectionStart()

        return scope.launch {
            val system = withContext(Dispatchers.IO) { promptBuilder.buildDirectionSystemPrompt() }
            val buildResult = withContext(Dispatchers.IO) {
                if (onlyThisRound) {
                    promptBuilder.buildReplyUserPromptOnlyThisRound(messages, userHint)
                } else {
                    promptBuilder.buildReplyUserPromptWithRefs(knowledgeBase, messages, userHint, false, intentConfig, corrections)
                }
            }
            val user = buildResult.prompt
            L.w("F07 direction prompt built, user=${user.length} chars")

            val providerConfig = deepSeekRepo.snapshotProviderConfig()
            if (providerConfig == null) {
                callbacks.onDirectionError("请先配置一个可用的模型供应商")
                callbacks.onDirectionEnd()
                return@launch
            }
            ...
            try {
                fullText = collectStream(
                    ...
                    onChunk = { chunk ->
                        buffer.append(chunk)
                        // 流式提取 directions 数组
                        val directions = parseDirectionsStreaming(buffer.toString())
                        if (directions.isNotEmpty()) {
                            callbacks.onDirectionStreaming(directions)
                        }
                    },
                    onError = { errorMsg = it }
                )
                ...
                if (e is kotlinx.coroutines.CancellationException) throw e
                errorMsg = e.message ?: "方向生成异常"
            }

            val finalSchemes = parseDirectionsStreaming(fullText.ifBlank { buffer.toString() })
            if (finalSchemes.isNotEmpty()) {
                callbacks.onDirectionResult(finalSchemes)
                L.w("F07 direction success: ${finalSchemes.count { it.reply.isNotBlank() }} non-empty")
            } else {
                val userFriendlyMsg = when {
                    errorMsg?.startsWith("PARAM_UNSUPPORTED:") == true ->
                        "模型不支持当前思考模式，请更换模型或关闭思考后再试"
                    errorMsg?.let { DeepSeekRepository.isConfigError(it) } == true ->
                        DeepSeekRepository.stripConfigPrefix(errorMsg.orEmpty())
                    else -> errorMsg ?: "方向生成失败，请重试"
                }
                callbacks.onDirectionError(userFriendlyMsg)
            }
            callbacks.onDirectionEnd()
        }
    }

    /** F07: 流式解析 directions 数组为 Scheme 列表 */
    private fun parseDirectionsStreaming(raw: String): List<Scheme> {
        val buffer = raw.replace("```json", "").replace("```", "")
        val start = buffer.indexOf("\"directions\"")
        if (start < 0) return emptyList()
        val arrStart = buffer.indexOf('[', start)
        if (arrStart < 0) return emptyList()

        val result = mutableListOf<Scheme>()
        for (dir in com.lovebrain.app.model.ReplyDirection.ALL) {
            // 按 ReplyDirection 顺序解析，每个方向取数组对应 index 的字符串
            // 简化：直接解析整个 directions JSON 数组
        }
        // 解析整个 directions 数组
        val arrEnd = buffer.indexOf(']', arrStart)
        if (arrEnd < 0) return emptyList()  // 数组未闭合
        val arrStr = buffer.substring(arrStart, arrEnd + 1)

        // 宽松解析字符串数组
        val items = runCatching {
            jsonLenient.decodeFromString<List<String?>>(arrStr)
        }.getOrNull() ?: return emptyList()

        for (dir in com.lovebrain.app.model.ReplyDirection.ALL) {
            val text = items.getOrNull(dir.index)?.takeIf { it.isNotBlank() } ?: ""
            result.add(Scheme(
                tag = dir.tag,
                title = dir.title,
                reply = text,
