...
/**
 * 真实提取节头正则（A9 计数口径修正）：与 extractLessons 写入格式 `# [yyyy-MM-dd HH:mm] 第N次提取` 逐字同构。
 ...
 * 知识库后台触发协调器。
 *
 * 职责：检查话题轮换阈值 → 串行执行经验提取 / 画像 reflect / 向量重估三引擎。
 *
 * 调度形态：冷流。本类**不接收 CoroutineScope、不 launch、不返回 Job**——
...
class KnowledgeTriggerCoordinator(
    ...
    /**
     * 检查阈值并串行执行三引擎：向量重估 → 经验提取 → 画像 reflect。
     *
     * 前一引擎完成才开始下一引擎；单引擎失败只发一条 [KnowledgeTriggerEvent.Notice]，不阻断后续。
     * reflect 放最后，天然拿到最新向量值。
     *
     * @param kbName 本轮话题轮换所属的知识库；所有事件都带这个身份，切库后由收集方决定丢弃
     */
    fun triggerEvents(kbName: String): Flow<KnowledgeTriggerEvent> = flow {
        // 不用 runCatching——它会吞掉 CancellationException
        try {
            val topicCount = knowledgeRepo.getLessonCount(kbName)
            if (topicCount <= 0) return@flow

            // 后台任务启动时冻结 corrections revision，写入前比对防迟到覆盖
            val frozenCorrectionsRev = knowledgeRepo.getCorrectionsRevision(kbName)

            if (topicCount % AppConfig.VECTOR_REESTIMATE_INTERVAL == 0) {
                reestimateVector(kbName, frozenCorrectionsRev) { emit(it) }
            }
            ...
            if (topicCount % AppConfig.LESSON_TRIGGER_INTERVAL == 0) {
                ...
                extractLessons(kbName, context, frozenCorrectionsRev) { emit(it) }
            }

            if (topicCount % AppConfig.REFLECT_TRIGGER_INTERVAL == 0) {
                generateReflect(kbName, frozenCorrectionsRev) { emit(it) }
            }
        } catch (e: kotlinx.coroutines.CancellationException) {
            throw e
        } catch (e: Exception) {
            L.e("triggerEvents failed", e)
        }
    }

    /**
     * 用户手动触发的画像重新生成（不依赖话题计数阈值）。
     *
     * 同样是冷流：调用方 cancel 会传播到底层模型请求、retry delay 与历史写入。
     */
    fun profileRefreshEvents(kbName: String): Flow<KnowledgeTriggerEvent> = flow {
        val frozenCorrectionsRev = knowledgeRepo.getCorrectionsRevision(kbName)
        generateReflect(kbName, frozenCorrectionsRev) { emit(it) }
    }

    /** 重估五维状态向量：读当前向量 + 最近上下文 → AI 重估 → 写回前校验 revision → 写回 warmth → 触发阶段建议 */
    private suspend fun reestimateVector(
        kbName: String,
        frozenCorrectionsRev: Int,
        emit: suspend (KnowledgeTriggerEvent) -> Unit
    ) {
        try {
            ...
            if (raw.isBlank()) {
                // 后台引擎失败轻提示（固定文案）
                emit(KnowledgeTriggerEvent.Notice(kbName, "向量重估本次失败，可稍后重试"))
                return
            }

            // 解析新向量：单维跳变超过 20 视为模型抖动，按 ±20 夹紧
            ...
            // 锁内原子 revision 检查 + 向量写入 + 历史记录，消除竞态窗口
            val ts = TimeFmt.now()
            val labelMap = mapOf(
            ...
            withContext(Dispatchers.IO) {
                knowledgeRepo.appendFileWithRevisionCheck(
                    kbName, "memory/vector_history.md", histEntry + "\n", frozenCorrectionsRev
                )
            }
            emit(
                KnowledgeTriggerEvent.VectorUpdated(
                    kbName = kbName,
                    newVector = newVector,
                    delta = newVector.mapValues { (k, v) -> v - (oldVector[k] ?: v) }
                )
            )
            ...
            if (changes.isNotEmpty()) {
                ...
                emit(KnowledgeTriggerEvent.VectorSummary(kbName, summary))
                val time = TimeFmt.now()
                withContext(Dispatchers.IO) {
                    knowledgeRepo.appendFileWithRevisionCheck(
                        kbName, "memory/reflect_history.md",
                        "\n\n### [$time] 五维向量变化\n$summary", frozenCorrectionsRev
                    )
                }
            }

            // 阶段建议（AI 建议的阶段与当前不同且非"维持"）——经 StageCatalog 归一化
            val suggested = StageCatalog.normalize(suggestedStage)
            if (suggested != null && suggested != currentStage) {
                emit(
                    KnowledgeTriggerEvent.StageSuggested(
                        StageSuggestion(
                            kbName = kbName,
                            newStage = suggested,
    ...
    /** 经验提取：把本轮话题上下文交给 lessons 引擎，非空且非"无新经验"才写库 */
    private suspend fun extractLessons(
        kbName: String,
        topicContext: String,
        frozenCorrectionsRev: Int,
        emit: suspend (KnowledgeTriggerEvent) -> Unit
        ...
        try {
            ...
            if (lessons.isNotBlank() && lessons != "无新经验") {
                // 锁内原子 revision 检查 + 追加经验，消除竞态窗口
                val existing = withContext(Dispatchers.IO) {
                    knowledgeRepo.readFile(kbName, "memory/lessons.md")
                }
                ...
                val written = withContext(Dispatchers.IO) {
                    knowledgeRepo.appendFileWithRevisionCheck(
                        kbName, "memory/lessons.md", entry, frozenCorrectionsRev
                    )
                }
                if (!written) {
                ...
                emit(KnowledgeTriggerEvent.Notice(kbName, "已自动提取新经验，记入知识库「经验」"))
            } else {
            ...
            L.e("extractLessons failed", e)
        }
    }

    /** 画像 reflect：分级自愈（正常 prompt → 严格 JSON → compact schema 重建），失败也出 typed 结果 */
    private suspend fun generateReflect(
        kbName: String,
        frozenCorrectionsRev: Int,
        emit: suspend (KnowledgeTriggerEvent) -> Unit
    ) {
        // 不用 runCatching——它会吞 CancellationException
        try {
...
$lastRaw"""
                ...
                val rawResult = try {
                    withContext(Dispatchers.IO) {
                        deepSeekRepo.generateRawWithMetadata(effectiveSystem, effectiveUser)
                    }
                } catch (e: kotlinx.coroutines.CancellationException) {
                    throw e
                } catch (e: Exception) {
                ...
                if (parseResult.status == ProfileParseStatus.SUCCESS) break
                ...
                // 所有可重试失败都要先 backoff 再进下一轮——delay 是 suspend，cancel 会传播
            ...
            if (finalResult == null || finalResult.status == ProfileParseStatus.EMPTY) {
                emit(KnowledgeTriggerEvent.Notice(kbName, "画像更新建议本次生成失败"))
                return
            }
            ...
            if (finalResult.status == ProfileParseStatus.SUCCESS && finalResult.profileUpdate != null) {
                ...
                emit(
                    KnowledgeTriggerEvent.ProfileReady(
                        ProfileSuggestion(
                            kbName = kbName,
                            display = display,
