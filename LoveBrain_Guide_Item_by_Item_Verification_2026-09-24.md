...
# 追加六十：两笔"注释比实现新"的假账销掉，顺手挖出两笔只登记没自裁的（`d6dc1a1`）

## 60.1 跨层存量的注释落后了两次还债

`PackageDependencyTest.kt` 的 KDoc 写着"今天的真实存量是 **15 条越界 import，分布在 11 个文件**"，
还特别加了一句"（数字来自 `bash scripts/package_deps_report.sh` 的一次实扫，不是凭印象写的）"——
**这句话现在两边都不成立**：

| 尺 | 实测（2026-09-26 本机） |
|---|---|
| `scripts/package_deps_report.sh --count` | **6** |
| 该文件自己的 `baseline` map | **6 个键、每个一条** |
| 该文件第 168 行 `assertEquals(…, 6, baseline.values.sumOf { it.size })` | 每次跑都在核这个 6 |

还债发生过**两次**，而且两次都在提交信息里写了数：`2dd94c0`（知识读写端口）15→10、
`ab7457a`（`AiGateway` 端口）10→6。**只有注释一个字没跟着动。**
同一段里还有第二处更细的：还债顺序写着"先……把 domain 的 6 处 `data.*` 换掉"——
那 6 处**已经换完了**（`ab7457a` 的标题就是"domain 不再 import data"），
现在 domain 只剩 `PromptBuilder` 一处 `android.content.Context`。
⇒ 两处都按实测改写，并把"15 是还债前的存量 + 那两个 SHA"留在注释里，
这样下一个人看见 6 不会以为债一直没动过。

## 60.2 验收包那句"仍用冻结 prompt"当场改口

指导书 §3.3（341 行）点名的两句话，`LoveBrain_Rework_Acceptance_2026-09-24.md` 里：
`:76` "漂移只记诊断、**仍用冻结 prompt**" 与 `:81` "**所有写路径统一拒**"。

- `:76` **改了**。这次是**对着实现核过**才改的，不是照抄指导书：
  `GenerationEngine.kt:256-264` 那段注释自己就写着
  "以前这行注释写的是'仍用冻结时的 prompt 文本'，那是没有实现的说法"；
  实读代码：hash 不一致只 `L.w` 一句（`:261-264`），紧接着 `:266` `val user = buildResult.prompt`
  送出去的就是**现读资产拼出来的那份文本**。⇒ "只记诊断"对，"仍用冻结 prompt"错，
  改口时把这条证据链写进那一格，并写明"要做到真冻结得把 `PreparedPrompt` 装进 `GenerationInput`
  （指导书给的另一个选项），至今没做"。
- `:81` **故意没动**（开工包 3.A7 原话："A1 修完之后第二句才真的成立，别先改字"，
  而 A1 那一格只造了尺、30 处裸写一处没还）。

## 60.3 顺手挖出来两笔，只登记、没自裁

1. **"改口"发生在哪一份文件上，`git log -- <那份文件>` 当场能证伪。**
   `LoveBrain_Three_Phase_Execution_Log_c0ff041_guide_2026-09-24.md:49` 那行写着
   `"仍用冻结 prompt"已改口`——读起来像验收包已经改过。实测：
   被改口的是**引擎里的注释**，而 `git log --oneline -- LoveBrain_Rework_Acceptance_2026-09-24.md`
   显示那份文件上一次被改动停在 `c0ff041`（比那行日志还早）。
   ⇒ 同一族教训的第 N 次：**"某处已改口"必须点名到文件**，注释改口 ≠ 文档改口
   （user 域那条"说做过了必须对着 diff 点名"的又一形态）。本格按 :76 真改了，
   但**没有回头去修那行日志的措辞**——它描述的是当时那一轮的处置，改它等于改历史。
2. **两份文档对同一句话的判决互相矛盾，不许我一方裁掉。**
   同一行 :49 还写着："'全部写路径统一拒绝' 现在**变成真的**了"，
   而开工包 3.A7 说 `:81` 那句**到 A1 还完债才成立**。口径分歧在哪：

   | 说法 | 判据 | 现在的读数 |
   |---|---|---|
   | 库内落盘"统一拒" | 34 处 `atomicWriteText` 调用全在 `atomicWriteText` 那道按 `kbOwning` 的判定下游（新尺 §59 数出来的） | **成立** |
   | "所有 mutation 只能从 `KnowledgeTx` 取得安全路径与原子写能力" | 唯一写链 4 / 裸写 30 | **不成立，30 处没还** |

   ⇒ 两个都对，只是一个说出口、一个说入口。**`:81` 那句话到底按哪个口径改，留给还完 30 处之后**，
   本格不择一自签。

## 60.4 死导入这一族又响了一次

`_temp/check_dead_imports.py` 扫"本格改过的文件"，当场扫出 `PackageDependencyTest.kt` 里
一条**在本格改动之前就已经死着**的 import：`org.junit.Assert.fail`（全文 0 次 `fail(`）。
⇒ 又一次证明 lint 抓不到死导入（同一族第 N 次，`8456199` 那格也是这么扫出来的）；
清掉之后那六格 `--rerun` 重跑一次全绿，死导入读数从 `5 explicit imports, 1 unused` 变成 `4 / 0`。

## 60.5 本机收口读数（`_temp/gates110/all.txt`，12 步全 rc=0）

- 单测：**190 套件 / 1417 单测 / 0 失败 / 0 错误 / 0 跳过**（`--rerun`）——
  与上一格（`aa29950`）**逐字相同**，本格只动注释与一条死导入，格子数一点没动，这正是期望的读数。
- lint：`measured 67 / 15 rules`、进预算 `66 / 14`、advisory 1 ⇒ 与开工单 §6 逐字相同；
  预算判据自测 27 格全对；跨层依赖 `--count` = **6**（就是 §60.1 那两个数）；工单号 PASS；
  prompt 零 diff（那一步 0 字节是它的通过信号）；资产锁 `6dcde732…` OK；
  取消审计 165 站（PROTECTED 54 / WAIVED 2 / SUSPEND-FREE 109 / NEEDS_REVIEW 0）；
  `assembleAndroidTest` rc=0；死导入：本格两个文件各 `4 explicit imports, 0 unused`。
- 门禁脚本换成 `_temp/run_gates110.sh`（死导入清单＝这一窗口改过的两个测试文件）。

## 60.6 这一格没做的

- `:81` 那句按上表的口径分歧**没改字**；执行记录 :49 那行措辞**没回头修**（它描述的是当时那一轮的处置）。
...
**4) 第四笔仪器账（`dd9368d`）**：三句 `requestCount` 断言打在 `isGenerating` 刚翻的那一帧上，
而连接要到 `PERF t2` 之后才建 ⇒ "还没发出"被报成"根本没发出"。
修完 fake 之前同一句读到的是 `accepted=1`：**旧 fake 吊住连接，反而盖住了这个竞态**。
⇒ 现在是先等再判（6 秒上限），三种结果各有各的脸；判决：**run 36234389326 已回，`tests=45 failures=0 errors=0 skipped=2`**——三句全部赶绿，而且连 `rapidDoubleTap`（KDoc 自陈"预期红"那格）也绿了：它报的不是生产竞态，是读太早。坑表 130。★ 但 `ui-test` job 仍红：证据门抓到 `home.png` 与 `knowledge-base.png` 逐字节相同（同 sha256、均 114996 字节）⇒ 第二块屏幕没换上来，这条上一轮账本已预告过；`upgrade-test` 因此继续 skipped。**发布判定仍 NO-GO**，但卡点已从"设备侧真失败"换成"视觉证据没拍到第二屏 + 升级测试永远 skipped + 两格跳过未交代"。
