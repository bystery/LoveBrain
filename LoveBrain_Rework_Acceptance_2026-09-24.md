...
| 7 | §6 S2-05 的"上帝类"这一项**只部分达成** | 部分 | `KnowledgeRepository` 已从 2283 降到 **1857**（低于报告基线 2118）：文本算法进 `KbTextOps`、schema 探测与旧库迁移进 `KnowledgeMigrator`。`LoveBrainViewModel` 2941 → **2651**（报告基线 2954，−10%）——赞踩/点踩案例进 `FeedbackCaseController`、纠正判定与文案进 `MemoryCorrectionPolicy`、改写台账与改写 prompt 进 `RewriteLedger`/`RewritePrompt`、方案卡位置映射进 `ReplyPatch`、两处生成指纹进 `GenerationFingerprints`，但**降幅只有 10%**，剩下的单条改写链路 228 行、锦囊 155 行、Trigger 事件落点 188 行还在同一个类里（持续意图的到期与保存校验已收进 `IntentPolicy`）。`PromptBuilder` 已从 1199 降到 **968**（低于基线 1177）。拆文件不等于拆职责，也不等于行数下降，这一格不给"完成"。 |
...
|---|---|---|
| S2-01 | Engine 立刻 `toChatMessages()/toKnowledgeBase()` 退回旧模型；`profile` 固定空串；revision 只是 turnCount 近似；准备后回读完整 config，不一致只写日志；prompt 资产版本没冻结 | 全部改掉：`KbContext.profile` 装真实画像正文（`readProfile`）；`revision` 是回复链路实际读取文件的 SHA-256（`contentRevision`），不再用 turnCount 近似；Provider 身份冻结 `ticketId/hostHash/model/thinkingMode` 四项，Engine 只按冻结 ticketId 取配置（`configForTicket`），**对不上就失败**（新增 `ReplyFailureKind.ProviderChanged`），不再"记日志然后用新配置发"；prompt 资产 hash 冻结进 input，漂移只记诊断（⚠ **2026-09-26 勘误**：原句这里写的是"仍用冻结 prompt"，那是**没有实现的说法**——`GenerationEngine.kt:256-264` 的注释自己已经改过口，本机实读是"hash 不一致只 `L.w` 一句，随后 `val user = buildResult.prompt` 送出去的就是**现读资产拼出来的那份文本**"；要真做到冻结构造好的文本，得把 `PreparedPrompt` 整体装进 `GenerationInput`，即指导书 §3.3 给的另一个选项，**至今没做**）；资产 hash 由 `asset_hashes.sh` + `docs/prompt-assets.lock` 落地并可 `--check`。`GenerationInput` 适配器仍保留但降级为 PromptBuilder 内部使用（公开入口收 GenerationInput） |
...
| `scripts/assert_artifacts.sh` | OK：真实 920 例 / 110 套件 / 0 failures / 0 errors / 0 skipped；空目录 → 1；篡改 XML 声明数 → 1 |
| `scripts/asset_hashes.sh --check` | OK：prompt 资产未漂移（`27abe25529deb47cb40366961998597c556c5184416fa038ff3ece81a2d6eb5c`） |
| `scripts/audit_cancellation.py --check` | OK：169 站点（PROTECTED 53 / WAIVED 2 / SUSPEND-FREE 114），NEEDS_REVIEW=0；注入吞取消样例 → exit 1（已验证会红） |
...
| +意图到期与保存校验外提 `IntentPolicy`（9 例） | 109 | 915 | 0 | 0 | 0 |
| +锦囊缓存判据外提 `SuggestCachePolicy`（5 例） | 110 | **920** | 0 | 0 | 0 |

本轮新增/改写的用例（逐套件读 XML，全部 0 失败；总数按最后一列为 920 例 / 110 套件）：
...
| `SuggestCachePolicyTest` | 5 | 四条判据各自单独失效都要 miss、没有缓存不算命中、老缓存缺 promptVersion 必然 miss、匹配是精确等值（不做前缀/忽略大小写） |
| `IntentPolicyTest` | 9 | TODAY 与 DATE 共用一条过期尺且当天不算过期、UNTIL_DONE 永不自动过期、只有启用+活动态才改状态、**没有日期的老意图不被悄悄判死**、过去日期只拦 ACTIVE、TODAY 一律把日期写成当天、连"今天"自己坏了也不误拒合法保存 |
...
`scripts/audit_cancellation.py --check` 169 站点中 NEEDS_REVIEW 0（其余三档是已放行/已豁免/段内无挂起点，
不是"没扫到"）；两者都用注入样例验证过"确实会红"（前者 `NEGATIVE-TEST exit=1`，后者 `NEGATIVE exit=1`）。

本轮最后一次全量门禁（HEAD = `403d8c3`，同一棵树）：
`:app:testDebugUnitTest` + `:app:lintDebug` + `:app:compileDebugAndroidTestKotlin` + `:app:assembleRelease`
→ BUILD SUCCESSFUL（920 例 / 110 套件 / 0 failures；lint 72 issue、0 error、0 fatal）；
...
3. S2-05 只算部分达成：`KnowledgeRepository` 这轮真的降到了基线以下（2118 → **1857**），
   但 `LoveBrainViewModel` 只从 2954 降到 **2651**（−10%），`TopicRecorder`(1006→991) 本轮未再动，
   我不给"职责拆分完成"这个词盖章。
...
报告给的被审行数与本轮 `wc -l` 实测（HEAD = `403d8c3`）：

| 文件 | 报告 | 上一轮 | 现在 | 变化 |
|---|---:|---:|---:|---|
| `LoveBrainViewModel.kt` | 2954 | 2941 | **2651** | −303（−10%，仍是第一行，见下）
...
`ReplyPatch.kt` 68、`GenerationFingerprints.kt` 96、`SceneChainInjection.kt` 96、`IntentPolicy.kt` 65、`PromptBudget.kt` 168；上一批还有
...
`LoveBrainViewModel` 剩下的块按节标记实测（同一把尺：节标记到下一个节标记的行距，HEAD = `403d8c3`）：
单条改写 228、Trigger 事件落点 188、今日锦囊 160、下一轮（存 KB + 清空）149、
事件归约 138、流式生成 134、回复状态唯一写入口 129、生成历史 122。
持续意图一节从 134 行降到 108 行（判定进 IntentPolicy），锦囊一节从 155 涨到 160
是因为缓存判据改成显式映射——那一刀换的是可测性，不是行数。
...
迁移器不得自己加锁）。本轮再把意图到期与保存校验收成一把尺（此前在发起生成、面板刷新、保存校验三处各写一遍，且填日期与判过去用了两个时钟）。报告点名的七个文件里，行数**现在全部低于**它在 `286c9406` 上测到的值
（2954/2118/1214/1177/1006/828/585 → 2651/1857/924/968/991/686/572）。
但这句话只到"行数"为止：ViewModel 仍有 2651 行、还同时管着回复状态机、轮次提交、
...
| 我自己写的哪一处 | 错在哪 | 处置 |
|---|---|---|
| 本文件 §1 与 §3 表里的"取消审计 167 站点 / PROTECTED 52" | 凭上一轮的记忆写的，生成件 `docs/CANCELLATION-AUDIT.md` 当时就是 **166 / 51**（表格行数与"合计"行自洽）——我在批评别人"凭自述写表"的同时自己写错了一个数 | 改成当次实测；本轮后续几刀把站点数推到 **169**（PROTECTED 53 / SUSPEND-FREE 114），数字来自 `audit_cancellation.py --check` 输出而不是我脑子里的数 |
