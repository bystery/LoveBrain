...
## 2l. 第十二轮：锦囊折叠入口 17dp → 48dp，读屏公告改说当前语言（`ca76bac`）

语义树第一次量到锦囊卡片那颗"点开看示例配文"的折叠入口：**344x17dp**
（320dp 宽 + 2.0 倍字时 304x33dp）——只有标题文字那么高。这是 P1-02 那一类缺陷
在第三个位置复现（前两个：三段切换 18dp、空态蓝字 23dp）。修法同前：
`heightIn(min = 48.dp)` + `Role.Button`，加 4 格用例（其中两格是行为断言：
公告要随状态变、展开后示例配文真的出现——不然"公告变了"可以只是一句空转）。

**读屏公告此前一直是中文硬编码**：`stateDescription = if (expanded) "已展开" else "已收起"`，
英文环境下 TalkBack 照样念中文。锦囊与谈心两处一起改走 strings（中英文各一份）。

为此给文案尺加了第三个判据 **STATE**，起点 0。这是继"锚点后面紧跟引号"之后发现的**第二个盲区**：
TEXT 与 DESC 两把尺都看不见 announce 给用户听的那句话——搬进资源之前，它连"欠了多少"都报不出来。
`SuggestTipCard` 由 private 改 internal（照 `ReplyPrimaryActions` 的先例）：
控件连被量的资格都没有，它的热区就是不可知的。

两笔过程错误值得记，因为都不是任务难度：
1. 第一版把 `stringResource(...)` 写进 `Modifier.semantics { }` → 编译不过，
   那个 lambda 不是 composable 上下文；改成在外面解析好再赋值。
2. **陈旧 XML 差点让我把结论读反**：上一轮构建失败后 `test-results/` 没清，
   下一次跑挂了别的过滤器，我差点把 5 分钟前那批红当成"修完还红"。
   现在每次读结果前按 mtime 比一次新鲜度（本轮 133 个套件全部 fresh，0 个被忽略）。

收尾实测：**1077 单测 / 133 套件 / 0 失败 / 0 跳过**；lint 71 条与预算一致；跨层 6；工单编号 PASS。
**仍欠**：谈心面板那颗"继续追问"胶囊（`CounselingPanel:362` 附近）文案已进资源，
但它整页要 `LoveBrainViewModel` 才能组合，本机测不到——那是 §5.2 第 6 步（VM 退成 facade）
挡住的第二个下游，不是"评估过不做"。

## 2m. 第十三轮：供应商页第一次被量到——两处修复 + 三处工具修正（`d902514`）

**「这页要 VM 所以测不了」不是理由。** `ProviderSection(viewModel, onBack)` 用 mockk 把
`tickets / activeTicket / providerReady` 三条 StateFlow 桩住就能整页组合。页面与 VM 之间的合同
就是那几条流；此前测不到其实是没人去量。

量到的两处：`＋ 添加供应商` 是 **312x34dp**（同类缺陷第五处），改 `heightIn(min=48)` + `Role.Button`；
折叠图标 `contentDescription = if (expanded) "收起" else "展开"` 是内联中文（英文环境照念中文），
连同「还没有供应商」「＋ 添加供应商」进 strings，实测公告现在念 Expand/Collapse。

**没做的说清：这一页还没换成 `ScreenState`**——空态仍是页面自己画的一行 `Text`。
下一步才是换（测试已铺好，换坏了当场会红）。

三处工具修正，全是被自家证据逼出来的：
1. 第一版测试点 `node[0]`，那是顶部返回箭头——三条断言全在「卡片根本没展开」的状态上跑。
   现在点最宽的那块，并加一格「展开后必须看到供应商条目」兜底。**假绿比红更糟。**
2. `SemanticsProbe.label` 优先取文字，合并节点里图标自己声明的 contentDescription 读不到，
   表现为假红 → `Target` 增加 `contentDescriptions` 单独可查。
3. 文案尺的**第三个**盲区：赋值型锚点的切片碰到 `if (...)` 的内层括号就提前收尾。
   线索是「我刚搬掉两处中文，DESC 却一个没变」——那个「没变」就是尺子在漏。
   修好后 DESC 实测 **12**（此前那次 10 同样是下界），并加 E.kt 夹具盯住这个形状。
   预算：TEXT 252→**250**、DESC 10→**12**、STATE **0**。

补记（`eb8666c`）：供应商页那句空态改走统一版式 `LbEmptyState`，并就地给出「添加供应商」（≥48dp + Role.Button 由组件保证）；页面下方那个同名的第二个入口一起收掉，加了一格断言盯住「空态里只能有一个添加入口」。一条边界要写死：**这一页整体仍没走 `ScreenState` 四态出口**（它自己 if 判空/非空），别把这次记成 §6.3 换完了。

再补一格（本提交）：供应商页的空/有内容两格并到同一个 `ScreenState` 判定 + 同一个 `LbAsyncState` 出口，页面里不再各画一遍。**这一格只用到 Empty/Content 两态**——这页没有 loading/error 流，不为了凑四态假造。改完 5 格用例仍全绿，于是把空判注成恒假做反证：「空态里只能有一个添加入口」那格当场红，跑完逐字节复原（sha `5d2666b4f64f8063` 前后一致）。

### §6.5 矩阵覆盖面扩到 12 格（本提交，纯测试侧）

`LbAsyncState`（统一空/错误态组件）与消息列表空态入口，从「只跑几格」改成
**4 宽 × 3 字 = 12 格全跑**：热区下限、可读标签、恰好一个入口，每格各断言一次。
两处都没量出新的越界（此前的修复已经把组件本身垫到位了），所以这一格买的是覆盖面，不是战果。

顺手给「矩阵循环」本身加了一行兜底断言：换配置若没真的进组合，12 格会量出同一个宽度、
而每条断言照样绿——所以用「同一宽度下 2.0 倍字必须比 1.0 倍字宽」当哨兵。
没有这行，"12 格全跑"只是一句自述。

收尾实测：**1082 单测 / 134 套件 / 0 失败 / 0 跳过**；lint 71 条与预算一致；工单编号 PASS。
