...
| 测试套件 | 测试类数 | 说明 |
|---|---|---|
| 数据层 | 8 | DeepSeek API 错误映射/配置分类、EventBus 捕获、HTTPS 信任、知识库安全/重入/备份、向量标签契约 |
| 领域层 | 5 | 知识引擎契约、消息截断、Prompt 组装顺序/阶段契约、阶段目录 |
| ViewModel | 9 | 花费展示、草稿持久化、想法提示、R2/R5 回归、画像确认、供应商就绪、工单切换、部分 JSON 解析 |
| UI | 1 | 对比度回归 |
| 工具 | 2 | JSON 工具、用量计价 |
| **合计** | **26** | 运行命令：`./gradlew test` |
